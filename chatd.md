...
A chat’s ownership state lets the chat worker decide whether a chat can be acquired or not. It’s decided by the `worker_id` and `runner_token` fields on the `chats` table. In total there are 2 ownership states.

| Shorthand | Worker ID | Runner token | Meaning      |
| --------- | --------- | ------------ | ------------ |
| `U`       | null      | null         | Unowned chat |
| `O`       | not null  | not null     | Owned chat   |

`worker_id` identifies the coderd replica that owns the chat. `runner_token` identifies one specific acquisition of the chat by a runner on that replica. Worker-owned transitions are fenced by `runner_token`, not by `worker_id`, so stale goroutines from an older acquisition cannot modify or abandon a newer acquisition by the same worker.
...
- `Create(initialUser)` creates a new chat, inserts its initial user turn, and lands in `running`.
- `DirectSend(m)` inserts one user message directly into an idle chat and lands in `running`.
...
- `Edit(k, replacement)` marks the truncated active-history suffix as deleted, inserts the replacement turn, and lands in `running`.
...
- `CancelRequiresAction(reason)` closes pending dynamic tool calls with synthetic cancellation tool results, satisfies the pending-action projection, clears `requires_action_deadline_at`, and lands in `running`. If the queue is empty it lands in `R0`; otherwise it lands in `R1`. It does not itself promote the queue head.

### Transitions used by the chat worker

Worker-owned transitions are fenced by `runner_token`. If the chat row's current `runner_token` does not match the caller's token, the transition fails without changing durable state. Worker transitions that depend on the current message history are also fenced by `history_version`. Transitions that commit generation output are additionally fenced by `generation_attempt`. Stale fence failures are expected concurrency outcomes.
...
- `Abandon(runner_token)` verifies that `runner_token` still owns the chat, then clears `worker_id`, `runner_token`, and `heartbeat_at`.
- `CommitStep(runner_token, history_version, generation_attempt, step)` inserts one durable message suffix while remaining `running`. A committed step may insert ordinary assistant/tool messages, and a compaction step may insert a compressed summary boundary plus visible compaction tool-call and tool-result messages.
- `EnterRequiresAction(runner_token, history_version, generation_attempt, calls)` records a pending-action episode by relying on the committed assistant tool-call messages as the durable call set, sets `requires_action_deadline_at`, which is a timestamp 5 minutes in the future, and lands in `requires_action`.
- `RunInterrupted(runner_token, history_version, generation_attempt?, optionalPartialStep)` inserts one final interrupted assistant/tool suffix if present, or finalizes interruption without a suffix if none is available, clears the interrupting state, and lands in `waiting` if no queued message is promoted. If interrupt finalization also promotes the queue head, it inserts the promoted queued message into history and lands in `running`. If a partial step is supplied, the transition also verifies `generation_attempt`.
- `RecordGenerationAttempt(runner_token, history_version)` verifies the chat is still `running`, increments `generation_attempt`, and returns the updated chat snapshot. If the incremented count exceeds the hardcoded attempt limit, the runner may later emit `FinishError(...)` instead of launching a fresh `GenerationSession`.
- `FinishTurn(runner_token, history_version, generation_attempt)` completes the current generation turn atomically. If the queue is empty, it lands in `waiting`. If the queue is non-empty, it removes the queue head, inserts it into history as a user turn, and lands in `running`.
- `FinishError(runner_token, history_version, generation_attempt, err)` ends a running chat in `error` and persists `last_error = err`, overwriting any prior stored error.
- `CancelRequiresAction(runner_token, history_version, reason)` closes pending dynamic tool calls with synthetic cancellation tool results, satisfies the pending-action projection, clears `requires_action_deadline_at`, and lands in `running`. Both the HTTP endpoints and the chat worker use the same durable transition, but worker calls are fenced by `runner_token` and `history_version`.
...
`worker_id` and `runner_token` have different purposes. `worker_id` identifies the coderd replica that acquired the chat, while `runner_token` identifies the specific local runner that owns this acquisition. Worker-owned transitions use `runner_token` as their durable fence so that stale goroutines from an older acquisition by the same replica cannot modify or abandon a newer acquisition.
...
`chats.history_version` stores the latest `snapshot_version` in which chat message history changed. It starts at `0`, remains unchanged for non-history transitions, and is set to the current `snapshot_version` whenever a message is inserted or updated.
...
BEGIN
  ...
  UPDATE chats
  SET history_version = snapshot_version
...
- from `A1`, if the promoted message is already head
  - endpoint: `A1 -> CancelRequiresAction -> R1 -> PromoteQueueHead -> R1/R0`
- from `A1`, if the promoted message is not already head
  - endpoint: `A1 -> ReorderQueue(qid, 0) -> A1 -> CancelRequiresAction -> R1 -> PromoteQueueHead -> R1/R0`
...
- `chat:ownership` is a global channel consumed by chat workers. Its payload is:
  ...
  It notifies chat workers about chats that need processing by a chat worker, but are not owned by a chat worker. A worker then attempts to acquire the chat.
...
Notifications are not authoritative. The database remains the source of truth for chat content, queue contents, status, worker ownership, and pending action state.
...
- `chat:ownership` is emitted when a transition leaves the chat in a runnable state, defined in [Acquisition loop](#acquisition-loop), and no worker owns it. That means either `worker_id` is NULL or `heartbeat_at` is expired.
...
For every chat with a `worker_id` that matches the worker's ID, the heartbeat loop updates the `heartbeat_at` field on the `chats` table to the current time every 9 seconds. 9 seconds is chosen so that a worker must miss 3 heartbeats before its lease on the chat expires, and the acquisition loop on another replica acquires its chat.
...
The manager is comprised of 2 loops.
...
- `CloseEpisode(chat_id, history_version, generation_attempt)`: closes an episode, preventing further parts from being added to it. May be called multiple times for a given episode, subsequent calls will be no-ops.
...
Subscribers must accept parts within 10 seconds of them being added to the buffer. If a subscriber does not accept a part within that timeframe, the subscription channel is closed.

In the future, we may consider making the limits admin-configurable, to allow tuning for high-scale deployments.
...
The runner is responsible for subscribing to the `chat:update:{chat_id}` pubsub channel.
...
1. If the event's `SnapshotVersion` is less than the runner's latest processed event's `SnapshotVersion`, ignore the event and stop.
2. If the event's `WorkerID` is not the runner's `WorkerID`, or the event's `RunnerToken` is not the runner's `RunnerToken`, or the event's `Archived` is `true`, send a cleanup request to the runner manager, and stop.
...
6. If the event indicates the core state machine is in `R0` or `R1` (status is `running`), spawn a goroutine to call the LLM API and execute tools, and mark it as active. We call this the **Generation goroutine**.
7. If the event indicates the core state machine is in `I0` or `I1` (status is `interrupting`), spawn a goroutine to handle the interrupt, and mark it as active. We call this the **Interrupt goroutine**.
8. If the event indicates the core state machine is in `A0` or `A1` (status is `requires_action`), spawn a goroutine to wait for the dynamic tool timeout to pass, and mark it as active. We call this the **Dynamic tools timeout goroutine**.
9. Otherwise, spawn a goroutine to abandon the chat. We call this the **Abandon chat goroutine**.
...
When receiving streaming message parts from the LLM API, the generation goroutine adds them to the [message part buffer](#message-part-buffer) in real time. Whenever it starts a new generation attempt, it must start a new episode in the buffer, and mark it as closed when the attempt is finished; either because the LLM API call returned a response, or the attempt was cancelled. If `AddPart` returns an error, the goroutine must retry with a new generation attempt.

Note: since the runner doesn't wait for goroutines to finish when it cancels them, and spawns new goroutines to perform new work immediately, the runner does not guarantee that any interrupted tool calls are fully stopped before continuing. Tool call interrupts are best-effort.

#### Interrupt goroutine

The interrupt goroutine is responsible for handling interrupts. It is spawned when the event indicates the core state machine is in `I0` or `I1` (status is `interrupting`). In addition to knowing its intended history version, chat status, and runner token, it also knows the generation attempt number that was interrupted.

The goroutine does the following in order:

1. It closes the episode corresponding to its intended history version and generation attempt number by calling the `CloseEpisode` method on the [message part buffer](#message-part-buffer).
2. It reads the buffered parts for that episode by calling the `GetParts` method on the message part buffer.
3. It applies the `RunInterrupted(partial?)` transition on the core state machine. If there are no buffered parts for that episode, it passes `nil` as the `partial` argument.

#### Dynamic tools timeout goroutine

The dynamic tools timeout goroutine is responsible for waiting for the dynamic tool timeout to pass, which is determined by the `requires_action_deadline_at` field on the chat. It is spawned when the event indicates the core state machine is in `A0` or `A1` (status is `requires_action`). When the timeout passes, it applies the `CancelRequiresAction` transition on the core state machine.
...
- This revision emits `message_part`, `message`, `status`, `queue_update`, `action_required`, `error`, and `preview_reset` events.
...
- instructing the [relay forwarder](#relay-mechanism) which streaming parts to forward;
...
- current `worker_id`;
- current `generation_attempt`;
- current requested preview episode:
  - `history_version`;
  - `generation_attempt`;
...
6. If the requested preview episode changes and the client may have preview state, emit `preview_reset`.
7. Configure the relay forwarder from `db.worker_id`, `db.history_version`, and `db.generation_attempt`.
8. Save local `snapshot_version`, `history_version`, `queue_version`, `status`, `worker_id`, and `generation_attempt`.
...
3. Set `local.history_version = db.history_version`.

If any fetched row is soft-deleted, mirror the current stream endpoint's full-refresh behavior:

1. Fetch all current non-deleted messages from the beginning in client-visible order.
2. Resend all messages as `message` events.
3. Replace the known-message revision map with the revisions from the resent messages.
4. Set `local.history_version = db.history_version`.

There is no separate client-visible full-refresh event. The full refresh is a server-side synchronization mode that causes the stream to resend the current visible message history from the beginning.
...
4. Set `local.queue_version = db.queue_version`.
...
2. If database status is `requires_action` and `db.history_version > local.action_required_history_version`, fetch the action-required payload.
3. Emit `action_required`.
4. Set `local.action_required_history_version = db.history_version`.
...
### Worker part buffering

The chat worker buffers live preview parts in memory.
...
- parts are grouped by:
  ...
  - `history_version`;
  - `generation_attempt`;
- each episode has monotonically increasing `seq`;
- the worker can replay buffered parts for a requested episode;
- the worker can stream future parts to subscribers waiting for that episode;
- buffered parts are deleted at least 30 seconds after the episode was last updated;
...
```text
history_version == local.requested_history_version
generation_attempt == local.requested_generation_attempt
...
mark that the client may have preview state
```
