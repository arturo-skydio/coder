...
export const useChatStore = (
	...
	const upsertCacheMessages = useCallback(
		(messages: readonly TypesGen.ChatMessage[]) => {
			...
			// the next idle refetch. Existing messages are also
			// covered (no ID comparison here), but the resulting
			// over-invalidation is harmless: chatPromptsQuery has a
			// 30s staleTime, and edits also explicitly invalidate via
			// the editChatMessage mutation.
